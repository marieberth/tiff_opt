 ...
 * NB: tsize_t is _int32 and not uint32 because some functions
 *     return -1.
...
typedef	_int32 tsize_t;		/* i/o size in bytes */
...
typedef void (*tileContigRoutine)
    (TIFFRGBAImage*, uint32*, uint32, uint32, uint32, uint32, _int32, _int32,
	unsigned char*);
...
typedef int (*TIFFErrnoProc)(void);
...
extern	int TIFFReadCustomDirectory(TIFF*, toff_t, const TIFFFieldInfo[], size_t);
...
extern void TIFFCIELabToXYZ(TIFFCIELabToRGB *, uint32, _int32, _int32,
			    float *, float *, float *);
extern void TIFFXYZToRGB(TIFFCIELabToRGB *, float, float, float,
			 uint32 *, uint32 *, uint32 *);

extern int TIFFYCbCrToRGBInit(TIFFYCbCrToRGB*, float*, float*);
extern void TIFFYCbCrtoRGB(TIFFYCbCrToRGB *, uint32, _int32, _int32,
			   uint32 *, uint32 *, uint32 *);
